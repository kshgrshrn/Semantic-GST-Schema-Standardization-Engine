...
# Fix seed so results are reproducible across runs
random.seed(42)
np.random.seed(42)

# The full 61-field canonical schema this system maps against
CANONICAL_HEADERS = [
    "SourceIdentifier", "SourceFileName", "GLAccountCode", "Division", "SubDivision",
    "ProfitCentre1", "ProfitCentre2", "PlantCode", "ReturnPeriod", "RecipientGSTIN",
    "DocumentType", "SupplyType", "DocumentNumber", "DocumentDate", "OriginalDocumentNumber",
    "OriginalDocumentDate", "CRDRPreGST", "LineNumber", "SupplierGSTIN", "OriginalSupplierGSTIN",
    "SupplierName", "SupplierCode", "POS", "PortCode", "BillOfEntry", "BillOfEntryDate",
    "CIFValue", "CustomDuty", "HSNorSAC", "ItemCode", "ItemDescriptiion", "CategoryOfItem",
    "UnitOfMeasurement", "Quantity", "TaxableValue", "IntegratedTaxRate", "IntegratedTaxAmount",
    "CentralTaxRate", "CentralTaxAmount", "StateUTTaxRate", "StateUTTaxAmount",
    "CessAmountAdvalorem", "CessRateSpecific", "CessAmountSpecific", "InvoiceValue",
    "ReverseChargeFlag", "EligibilityIndicator", "CommonSupplyIndicator", "AvailableIGST",
    ...
    "PaymentVoucherNumber", "PaymentDate", "ContractNumber", "ContractDate", "ContractValue"
]

# Real-world noise patterns pulled from accounting data — abbreviations,
# reorderings, and casing variations commonly seen in client-submitted files.
NOISE_PATTERNS = {
...
def build_test_set(headers):
    """
    Generates a deterministic set of synthetic header variations per canonical field.
    Each field gets: exact match, space-cased lowercase, ALL CAPS, and up to 2 abbreviation swaps.
    Returns list of (noisy_header, ground_truth) tuples.
    """
    test_cases = []
    for true_col in headers:
        ...
        # lowercase with spaces — e.g. "SupplierName" -> "supplier name"
...
def run_benchmark(model, test_cases, schema_embeddings):
    top1, top3 = 0, 0
    cosine_scores = []
...
def print_results(label, results, n, elapsed):
...
def main():
    ...
    n = len(test_cases)
    print(f"\nTest set built: {n} synthetic permutations across {len(CANONICAL_HEADERS)} canonical fields (seed=42)")
    print("=" * 60)

    # --- Baseline: untuned all-MiniLM-L6-v2 ---
